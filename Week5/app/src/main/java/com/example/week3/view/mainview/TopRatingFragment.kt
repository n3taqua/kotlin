...
class TopRatingFragment(layoutManagerSpanCount:Int=1)  : Fragment() {
    ...
    private lateinit var rv_adapter_list_film:rv_adapter
    ...
    private fun getDataReCyclerView() {
        ...
        rv_adapter_list_film= rv_adapter(rv_layoutManager)
        movieFavoriteViewModel.listMovieFavorite.observe(viewLifecycleOwner, Observer {
            rv_adapter_list_film.setListFavoriteMovie(it)
            rv_adapter_list_film.setListMovie(movieFavoriteViewModel.listAllMovie.sortedBy { it.vote_average } as MutableList<Movie>)
        })
    }

    private fun setRecyclerView() {
        rv_list_film_top_rating.layoutManager=rv_layoutManager
        rv_list_film_top_rating.adapter=rv_adapter_list_film
